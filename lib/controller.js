...
controller.editUser = function (data, callback) {
  fs.readFile(
    `./../main/data/users/${data.id}.json`,
    "utf8",
    function (err, _data) {
      if (!err && _data) {
...
controller._checkId = function (id, callback) {
  fs.readFile(`./../main/data/users/${id}.json`, "utf8", function (err, _) {
    if (err) {
      callback(false);
    } else {
      callback(true);
...
controller.deleteUser = function (data, callback) {
  console.log(data);
  if (data.id) {
    controller._checkId(data.id, function (boolean) {
      if (boolean) {
        fs.unlink(`./../main/data/users/${data.id}.json`, function (err) {
          if (!err) {
...
controller.menu = function (callback) {
  fs.readFile("./../main/data/menu/menu.json", "utf8", function (err, data) {
    if (!err && data) {
      callback(false, data);
    } else {
      callback(err);
