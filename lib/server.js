    ...
    if (pathName === "makeUser") {
      ...
      if (data.payload) {
        ...
        controller.makeUser(outputData, function (err, msg = "") {
          if (!err) {
            // prettier-ignore
            res.write(`<h1 class="sucMessage">your id is: <span class="User-ID">${outputData.id}</span></h1>`);
            res.end();
          } else {
            console.log(err);
            res.end();
      ...
      console.log(render.template(`${__dirname}/template/editUser.html`, res));
      // if (render.template(`${__dirname}/template/editUser.html`, res)) {
      //   res.end();
      // }
      // res.write("hi there ");
      //
      ...
      render.template(
        `${__dirname}/template/deleteUser.html`,
        res,
        "deleteUser"
      );
